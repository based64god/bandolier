...
func run(ctx context.Context, cfg config) error {
	...
	// dashboard dims harness lines; an untagged multi-line block would render as
	// Claude output).
	...
	if prBranch != "" {
		// For dashboard (non-issue) PRs, title from Claude's commit summary rather
		// than the prompt. Issue PRs keep the issue title (they close the issue).
		if cfg.issueNumber == "" {
